...
public class NameFinderDemo
{
   public static void main (String[] args) throws IOException
   {
      ...
      //{
         ...
         Person[] people = {new Person("Makar", "Alexeevich"), new Person("Joseph", "Bazdeev"), new Person("Boris", "Drubetskoy")};
         ...
         while (stdin.hasNextLine())
         {
            ...
            {
               pw.format("%5d  %8d  %s\n", lineNumber, (1 + line.indexOf(people[0].getFullName())), people[0].getFullName());
            }
            if (lastWord.equals(people[0].getFirstName()) && stdin.hasNext(people[0].getLastName()))
            {
               pw.format("%5d  %8d  %s\n", lineNumber, (1 + line.indexOf(people[0].getFirstName())), people[0].getFullName());
            }

            // JOSEPH
            if (line.contains(people[1].getFullName()))
            {
               pw.format("%5d  %8d  %s\n", lineNumber, (1 + line.indexOf(people[1].getFullName())), people[1].getFullName());
            }
            if (lastWord.equals(people[1].getFirstName()) && stdin.hasNext(people[1].getLastName()))
            {
               pw.format("%5d  %8d  %s\n", lineNumber, (1 + line.indexOf(people[1].getFirstName())), people[1].getFullName());
            }

            // BORIS
            if (line.contains(people[2].getFullName()))
            {
               pw.format("%5d  %8d  %s\n", lineNumber, (1 + line.indexOf(people[2].getFullName())), people[2].getFullName());
            }
            if (lastWord.equals(people[2].getFirstName()) && stdin.hasNext(people[2].getLastName()))
            {
               pw.format("%5d  %8d  %s\n", lineNumber, (1 + line.indexOf(people[2].getFirstName())), people[2].getFullName());
            }
         }
         ...
         pw.close();
         //sumPerformanceTimes += (endTime - startTime );
      //}
      //System.out.println("Average Performance Time: " + sumPerformanceTimes / 100);
   }
//
}
